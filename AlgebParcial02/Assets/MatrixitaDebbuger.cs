...
public class MatrixitaDebbuger : MonoBehaviour
{
	public GameObject target;
	GUIStyle style = new GUIStyle();
	[MenuItem("Window/MatrixitaDebugger")]
	static void Init()
	{
	...
	private void OnGUI()
	{
		...
		if (target != null)
		{
			...
			DrawMatrix(Matrixita4x4.Translate(target.transform.position));
			EditorGUILayout.Space();
			EditorGUILayout.LabelField("Rotation matrix", style);
			DrawMatrix(Matrixita4x4.Rotate(target.transform.rotation));
			EditorGUILayout.Space();
			EditorGUILayout.LabelField("Scale matrix", style);
			DrawMatrix(Matrixita4x4.Scale(target.transform.localScale));
			EditorGUILayout.Space();
			EditorGUILayout.LabelField("TRS matrix", style);
			DrawMatrix(Matrixita4x4.TRS(target.transform.position, target.transform.rotation, target.transform.localScale));
		}
	}

	private void DrawMatrix(Matrixita4x4 matrix)
	{
		GUIStyle style = new GUIStyle();
		style.fontSize = 20;
